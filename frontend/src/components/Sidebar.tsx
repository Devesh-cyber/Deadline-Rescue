...
export default function Sidebar({ currentView, setView, onRescueClick, userProfile, hasActiveTask }: SidebarProps) {
  return (
    <aside className="hidden md:flex flex-col h-screen w-64 border-r border-outline-variant bg-[#0A0A0A] flex-shrink-0">
      <div className="flex flex-col h-full py-8 px-5">
        ...
        <nav className="flex-1 space-y-2">
          ...
          <button
            onClick={() => hasActiveTask && setView("dashboard")}
            disabled={!hasActiveTask}
            className={`w-full flex items-center justify-between px-4 py-3.5 transition-all duration-200 text-left font-sans ${
              currentView === "dashboard"
                ? "text-black font-bold bg-primary"
                : (hasActiveTask ? "text-on-surface-variant hover:text-white hover:bg-white/5 cursor-pointer" : "text-on-surface-variant/30 cursor-not-allowed")
            }`}
            title={!hasActiveTask ? "Select a task from Tasks Hub first" : ""}
          >
            <div className="flex items-center gap-3">
              <LayoutDashboard
                size={18}
                className={
                  currentView === "dashboard"
                    ? "text-black"
                    : (hasActiveTask ? "text-white/40" : "text-white/10")
                }
              />
              <span className="text-xs uppercase tracking-widest font-semibold">
                Dashboard
          ...
          <button
            onClick={() => hasActiveTask && setView("execution")}
            disabled={!hasActiveTask}
            className={`w-full flex items-center justify-between px-4 py-3.5 transition-all duration-200 text-left font-sans ${
        ...
        <div className="mb-6 p-4 bg-[#111111] border border-outline-variant rounded-xl">
          ...
          <ul className="space-y-2 text-[11px] text-white/60 font-sans">
            <li className="flex gap-2">
              ...
              <span>Select a project from the Hub to begin execution.</span>
            </li>
            <li className="flex gap-2">
              <span className="text-primary">•</span>
              <span>Use New Task to parse fresh assignments.</span>
            </li>
          </ul>
        </div>
        ...
        <div className="mt-auto border-t border-outline-variant pt-5">
          <button
            ...
            className="w-full bg-transparent hover:bg-primary border border-white/20 hover:border-primary hover:text-black text-white py-3 px-4 font-mono font-bold text-xs uppercase tracking-widest active:scale-95 transition-all duration-200 cursor-pointer mb-5 rounded-lg"
          >
